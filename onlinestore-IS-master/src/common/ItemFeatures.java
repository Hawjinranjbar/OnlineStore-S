...
public class ItemFeatures {
    ...
    public static ItemFeatures fromString(String row) {
        ...
        if (parts.length != 5) throw new IllegalArgumentException("Invalid item features row");
        int productId = Integer.parseInt(parts[0]);
        String brand = parts[1];
        String description = parts[2];
        String skinType = parts[3];
        boolean isOrganic = Boolean.parseBoolean(parts[4]);

        return new ItemFeatures(productId, brand, description, skinType, isOrganic);
    }
}
